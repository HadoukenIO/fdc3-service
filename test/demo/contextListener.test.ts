...
describe('advanced puppeteer functionality', () => {
  ...
  it('can can register and trigger a context listener', async () => {
    ...
    expect(fin.Application.wrapSync(testManagerIdentity).isRunning())
        .resolves.toBe(true);
    ...
    expect(fin.Application.wrapSync(testIdentity1).isRunning())
        .resolves.toBe(true);
    expect(fin.Application.wrapSync(testIdentity2).isRunning())
        .resolves.toBe(true);
    ...
    const callbackResponse =
        fdc3Remote.addContextListener(testIdentity2, callbackSpy);
    await new Promise(
        res => setTimeout(
            res,
