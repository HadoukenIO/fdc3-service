 ...
 * each intent.
 *
 * These preferences can be both global and per-application. If both exist,
 * per-application preferences will take priority. This store deals with FDC3
 * application id's (as found in the app directory), and not OpenFin UUID's.
 *
 * Methods within this class are null-tolerant. Passing null for any
 * UUID/directoryId will result in null being returned.
 *
 * Note that user preferences aren't currently saved to any kind of permanent
