...
export abstract class AbstractAppWindow implements AppWindow {
    ...
    public isReadyToReceiveIntent(intent: IntentType): Promise<boolean> {
        return this.isListenerReady(this._onIntentListenerAdded, () => this.hasIntentListener(intent));
    }

    public isReadyToReceiveContext(): Promise<boolean> {
        return this.isListenerReady(this._onContextListenerAdded, () => this.hasContextListener());
    }

    public isReadyToReceiveContextOnChannel(channel: ContextChannel): Promise<boolean> {
    ...
    private isListenerReady<A extends any[]>(listenerAddedSignal: Signal<A>, hasListenerPredicate: () => boolean): Promise<boolean> {
        ...
        return untilTrue(listenerAddedSignal, hasListenerPredicate, rejectOnMaturePromise).then(() => true, () => false);
    }
}
