 ...
 * results will be cached and then updated periodically.
 *
 * This directory currently uses a local file as the application directory. Once
 * fully developed, the application directory will exist as a remote service.
 */
export class AppDirectory {
  ...
  private static URL: string =
      'http://localhost:3923/provider/app-directory.json';

  /**
   * If cached application list is older than this duration, the next request to
   * fetch the list of applications will skip the cache and go to the API.
   */
  private static MAX_CACHE_AGE: number =
  ...
  public async getApplications(): Promise<IApplication[]> {
    if (this.applications &&
        Date.now() < this.timestamp + AppDirectory.MAX_CACHE_AGE) {
  ...
  private async request(): Promise<IApplication[]> {
    return new Promise(
        (resolve: (value: IApplication[]) => void,
          ...
          xhr.onreadystatechange = () => {
            if (xhr.readyState === XMLHttpRequest.DONE) {
              if (xhr.status >= 200 && xhr.status < 400) {
                ...
                reject(new Error(
                    'AppDirectory: Unexpected status (' + xhr.status + ')'));
              }
            }
          };
          xhr.send();
