...
export class FDC3 {
  ...
  public async register(): Promise<void> {
    ...
    const service =
        await fin.InterApplicationBus.Channel.create(SERVICE_CHANNEL);
    // handle client connections
    service.onConnection((app, payload) => {
      if (payload && payload.version && payload.version.length > 0) {
        console.log(
            `connection from client: ${app.name}, version: ${payload.version}`);
      } else {
        console.log(
            `connection from client: ${app.name}, unable to determin version`);
      }
    });
    ...
    service.register(
        FDC3.MSG_SELECTOR_RESULT, this.onSelectorResult.bind(this));
  }
  private async onOpen(payload: IOpenArgs): Promise<void> {
    const applications: IApplication[] = await this.directory.getApplications();
    const requestedApp: IApplication|undefined =
        applications.find((app: IApplication) => app.name === payload.name);
    return new Promise<void>(
        (resolve: () => void, reject: (reason: Error) => void) => {
          if (requestedApp) {
            this.openApplication(requestedApp, payload.context)
                .then(resolve, reject);
          } else {
  ...
  private async onResolve(payload: IResolveArgs): Promise<IApplication[]> {
    ...
    if (payload.intent) {
      ...
      return applications.filter(
          (app: IApplication) => app.intents.includes(payload.intent));
    } else {
  ...
  private async onIntent(intent: Intent, source: Identity): Promise<void> {
    let applications: IApplication[] =
        await this.onResolve({intent: intent.intent, context: intent.context});
    ...
    if (applications.length === 1) {
      ...
      return this.onOpen({name: applications[0].name}).then(() => {
        return this.sendIntent(
            intent, this.metadata.lookupFromDirectoryId(applications[0].id)!);
      });
    } else if (applications.length > 1) {
      // Ask user to manually select an application
      return this.resolveIntent(intent, source, applications)
          .then((selectedApp: IApplication) => {
            return this.sendIntent(
                intent, this.metadata.lookupFromDirectoryId(selectedApp.id)!);
          });
    } else {
   ...
   */
  private applyIntentPreferences(
      intent: Intent, applications: IApplication[],
      source: Identity): IApplication[] {
    // Check for any explicit target set within the intent
    if (intent.target) {
      const preferredApplication: IApplication|undefined =
          applications.find((app: IApplication) => app.name === intent.target);
      if (preferredApplication) {
        return [preferredApplication];
      }
    }
    // Check for any user preferences
    const preferredAppId: number = this.preferences.getPreferredApp(
        this.metadata.mapUUID(source.uuid)!, intent.intent)!;
    if (applications.length > 1 && preferredAppId) {
      const preferredApplication: IApplication|undefined =
          applications.find((app: IApplication) => app.id === preferredAppId);
      if (preferredApplication) {
      ...
      if (result.success && result.app) {
        ...
        switch (result.defaultAction) {
          case eDefaultAction.ALWAYS_FOR_INTENT:
            this.preferences.setGlobalPreference(
                queuedIntent.intent.intent, result.app.id);
            break;
          case eDefaultAction.ALWAYS_FOR_APP:
            if (queuedIntent.source) {
              this.preferences.setAppPreference(
                  queuedIntent.source.directoryId, queuedIntent.intent.intent,
                  result.app.id);
            }
            break;
          default:
   ...
   */
  private async openApplication(requestedApp: IApplication, context?: Payload):
      Promise<void> {
    return new Promise<void>((resolve, reject) => {
      const metadata: IAppMetadata|null =
          this.metadata.lookupFromDirectoryId(requestedApp.id);
  ...
  private async startApplication(appInfo: IApplication, context?: Payload):
      Promise<void> {
    return new Promise<void>(
        (resolve: () => void, reject: (reason: Error) => void) => {
          if (appInfo) {
            fin.Application.createFromManifest(appInfo.manifest_url)
                .then((app) => {
                  ...
                  const timeout = setTimeout(() => {
                    console.warn(
                        'Timeout whilst waiting for application to start');
                    reject(new Error(
                        'Timeout whilst waiting for application to start: ' +
                        appInfo.name));
                  }, 15000);
                  ...
                  app.run()
                      .then(() => {
                        ...
                        if (context) {
                          ...
                          fin.InterApplicationBus.publish('context', {context})
                              .then(resolve)
                              .catch(
                                  (reason: string) =>
  ...
  private async resolveIntent(
      intent: Intent, source: Identity,
      applications: IApplication[]): Promise<IApplication> {
  ...
  private async openAppSelector(queuedIntent: IQueuedIntent):
      Promise<fin.OpenFinApplication> {
  ...
  private async sendIntent(intent: Intent, targetApp: IAppMetadata):
      Promise<void> {
    if (targetApp) {
      ...
      // to all applications
      console.warn(
          'No target given for intent. Going to broadcast to all applications');
        ...
        (resolve: (value: boolean) => void,
         reject: (reason: string) => void) => {
          fin.desktop.System.getAllApplications(
              (applicationInfoList: fin.ApplicationInfo[]) => {
                resolve(!!applicationInfoList.find(
                    (app: fin.ApplicationInfo) =>
                        app.uuid === uuid && app.isRunning));
              },
              reject);
        });
  }
