...
import {APIFromClientTopic, getServiceChannel, setServiceChannel, APIFromClient, deserializeError, Events, setServiceIdentity, getServiceIdentityUUID, getServiceIdentityName} from './internal';
            ...
            channelPromise = new Promise<ChannelClient>((resolve, reject) => {
                fin.System.getRuntimeInfo().then((info: RuntimeInfo & {fdc3AppUuid?: string; fdc3ChannelName?: string}) => {
                    if (info.fdc3AppUuid && info.fdc3ChannelName) {
                        setServiceIdentity({uuid: info.fdc3AppUuid, name: info.fdc3AppUuid});
                        setServiceChannel(info.fdc3ChannelName);
                    }
                    if (fin.Window.me.uuid === getServiceIdentityUUID() && fin.Window.me.name === getServiceIdentityName()) {
